...
function spare_init_gateway_class()
{
    ...
    class WC_Spare_Gateway extends WC_Payment_Gateway
    {
        ...
        public function process_payment($order_id)
        {
            ...
            $total = strval($order->get_total() + 0);
            $arr = array('amount' => $total, 'description' => $this->description);
            $ecdsa = new Helpers\Security\DigitalSignature\EccSignatureManager();
            $signature = $ecdsa->Sign(json_encode($arr), $this->private_key);   
            $rep = $client->CreateDomesticPayment(new \Payment\Models\Payment\Domestic\SpDomesticPayment(
                $order->get_total(), $this->description
            ), $signature);


            $createdPayment = $rep->getPayment();

            $woocommerce->cart->empty_cart();

            return array(
                'result' => 'success',
                'redirect' => $createdPayment['link']
            );
        }
    }
}   
